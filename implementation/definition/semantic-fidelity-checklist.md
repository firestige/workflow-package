...
Machine Definition: `workflow-package/implementation/definition/` (`agentops.workflow-dsl@2.0.0`, Package `0.4.4`). The semantic source remains `workflow.md`; this checklist records the 2.0 structural migration and does not create new Workflow meaning. Status: `DESIGN_REFERENCE`.
...
Expected results: schema/graph/event/authority/corpus-shape/digest closure passes; the migration guard proves DSL 2.0 versions, removed 1.x bindings, exact Role/Action prompts and the System Design 0.4.4 handoff coordinate.
